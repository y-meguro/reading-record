...
- 詳細な実装は、自力でやったと言うより、ほぼほぼ提供されているコードを参考に記載した
- 前半の理論のところでも適宜ななめ読みで進めたので、実装時につまずいたタイミングで改めて振り返って学習した

## 読む前後の変化

- OS がどのような作業をしているかイメージが明確になった
  - BIOS の役割
  - リアルモードとプロテクトモードの違い、移行方法
  - 割り込み / タスクスイッチ / ページング
  - 周辺機器の制御
  - 数値演算コプロセッサの使い方
  - ファイルシステム
...
- ボリュームの多い本だが、丁寧に解説されているので、1歩ずつ進めていける本だと思う
  - また、お手本のコードが提供されているので、とても参考になる
- 読み手が何を期待するかで、進め方の工夫をするといい本だと思った。自分の場合、自力でアセンブリ言語をごりごり書いていくのが厳しかったので、何をやっているか意識しながらコピペ中心で進めていった
  - アセンブリの詳細については学習できていないが、適宜デバッグが必要になったりするので、概観は十分学習できると思う
- 初版を読んだこともあって、たまに記載に間違いがある。サポートページを見ながら進めていくといいと思う
...
- プロテクトモードからリアルモードへ移行できたが、まだ BIOS コールを実行できない。プロテクトモード移行時に、割り込みコントローラの設定が変更されているため
- 起動時の割り込み設定を復元する
- ファイルを検索する

## 27章: PC の電源を切る

- ソフトウェアでパワーオフ（電断処理）を実現するには、システム状態を S5 に遷移させる
- 電断契機を検出する
- 電断処理を実装する
